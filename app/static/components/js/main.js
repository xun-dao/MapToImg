    ...
    return function (s) {
        s.setup = function () {
            ...
            for (var i = 0; i < max_colors; i++) {
                gfg = s.createGraphics(canvas_size, canvas_size);
                // gfg.background(0);
                // gfg.fillStyle = "rgba(0,0,255,0.5)";
                s.mask.push(gfg);
            }
...
function updateResult() {
    ...
    $.ajax({
        type: "POST",
        url: "/post",
        data: JSON.stringify({ "id": id, "references": palette, "data_reference": data_reference, "colors": colors }),
        dataType: "json",
